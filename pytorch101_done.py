...
def multiples_of_ten(start, stop):
    ...
    while start <= stop:
        ...
        start += 10
    x = torch.tensor(ans_list, dtype=torch.float64)
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
    return x
...
def normalize_columns(x):
    ...
    # Replace "pass" statement with your code
    len = x.shape[0]
    average_tensor = x.sum_to_size(len) / len
    subtract = x - average_tensor
    square = subtract**2
    square_sum = square.sum_to_size(len) / (len - 1)
    sqrt_tensor = square_sum.sqrt()
    y = subtract / sqrt_tensor
