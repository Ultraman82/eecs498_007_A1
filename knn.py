...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    ...
    for j in range(num_test):
        test_ = x_test[j]        
        distance = (train_ - test_).pow(2)
        dists[i][j] = distance.sum().sqrt()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  test_set = torch.flatten(x_test, start_dim=1)
  for i in range(num_train):     
    train_ = torch.flatten(x_train[i])    
    distance = (train_ - test_set).pow(2)
    sumup = distance.sum(dim=1).sqrt()
    dists[i] = sumup
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
